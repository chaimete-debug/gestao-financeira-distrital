...
function normalizeTipoMovimento(tipo){
  const t=String(tipo||'').trim().toUpperCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'');
  if(['ENTRADA','RECEITA','REC','E'].includes(t)) return 'ENTRADA';
  if(['SAIDA','DESPESA','DES','S'].includes(t)) return 'SAIDA';
  return t;
}
function isEntrada(tipo){ return normalizeTipoMovimento(tipo)==='ENTRADA'; }
function isSaida(tipo){ return normalizeTipoMovimento(tipo)==='SAIDA'; }
function labelTipoMovimento(tipo){ return isEntrada(tipo)?'Entrada':isSaida(tipo)?'Saída':safeText(tipo); }
...
function openDashboard(){
  ...
  document.getElementById('contentArea').innerHTML=`
    <div class="grid" style="margin-top:0;">
      ...
      <div class="card"><div class="card-title">📥 Entradas Submetidas</div><div id="dashEntradasSub" class="card-value">0,00 MZN</div></div>
      <div class="card"><div class="card-title">📤 Saídas Submetidas</div><div id="dashSaidasSub" class="card-value">0,00 MZN</div></div>
      <div class="card"><div class="card-title">🧮 Saldo Projectado</div><div id="dashSaldoProj" class="card-value">0,00 MZN</div></div>
    ...
    <div class="info" style="margin-top:10px;padding:10px 12px;border-radius:10px;font-size:12px;line-height:1.4;">
      ℹ️ O <strong>Saldo Aprovado</strong> considera apenas lançamentos aprovados. O <strong>Saldo Projectado</strong> soma também os lançamentos submetidos, ainda pendentes de aprovação.
    </div>
  ...
  serverCall('Dashboard_getSummary',[{}],function(s){
    const entradasAprovadas = Number(s?.entradas_aprovadas ?? s?.receitas_aprovadas ?? 0);
    const saidasAprovadas   = Number(s?.saidas_aprovadas ?? s?.despesas_aprovadas ?? 0);
    const entradasSub       = Number(s?.entradas_submetidas ?? s?.receitas_submetidas ?? 0);
    const saidasSub         = Number(s?.saidas_submetidas ?? s?.despesas_submetidas ?? 0);
    const saldoAprovado     = Number(s?.saldo ?? 0);
    const saldoProjectado   = Number(s?.saldo_projectado ?? (saldoAprovado + entradasSub - saidasSub));

    document.getElementById('dashEntradas').textContent    = formatMoney(entradasAprovadas);
    document.getElementById('dashSaidas').textContent      = formatMoney(saidasAprovadas);
    document.getElementById('dashSaldo').textContent       = formatMoney(saldoAprovado);
    document.getElementById('dashEntradasSub').textContent = formatMoney(entradasSub);
    document.getElementById('dashSaidasSub').textContent   = formatMoney(saidasSub);
    document.getElementById('dashSaldoProj').textContent   = formatMoney(saldoProjectado);
    ...
    document.getElementById('kpiSaldo').textContent=formatMoney(saldoAprovado);
...
function loadDashDeptos(){
  ...
  serverCall('Dashboard_resumoDeptos',[{ano,mes}],function(res){
    ...
    if(!linhas.length){ wrap.innerHTML=`<div class="info">ℹ️ Sem lançamentos aprovados em ${periodo}${igNome}. Os lançamentos submetidos aparecem nos cartões do dashboard, mas só entram neste resumo depois da aprovação.</div>`; return; }
  ...
  serverCall('Lancamento_listAll',[filtros],function(res){
    ...
    wrap.innerHTML=`<div style="overflow-x:auto;-webkit-overflow-scrolling:touch;"><table>
      ...
      <tbody>${rows.map(l=>`<tr>
        ...
        <td style="white-space:nowrap;">${isEntrada(l.tipo_movimento)?'📈 Entrada':'📉 Saída'}</td>
...
function actualizarRubricas(){
  ...
  serverCall('Lancamento_getRubricasPorGrupo',[idGrupo],function(arr){
    ...
    const fil=lista.filter(r=>!r.tipo_movimento||!tipo||normalizeTipoMovimento(r.tipo_movimento)===normalizeTipoMovimento(tipo));
...
function guardarLancamento(modo){
  const tipo    =normalizeTipoMovimento(document.getElementById('fl_tipo')?.value);
...
function gerarRelatorioRubricas(){
  ...
  serverCall('Lancamento_listAll',[{estado:'APROVADO', mesInicio, mesFim, ano}],function(rows){
    ...
    filtrados.forEach(function(l){
      ...
      if(isEntrada(tipo)) agrupado[idRub].entrada+=valor;
      if(isSaida(tipo))   agrupado[idRub].saida+=valor;
    });
...
function tabelaRubricas(linhas,res){
  return `<div style="overflow-x:auto;-webkit-overflow-scrolling:touch;"><table>
    ...
    <tbody>${linhas.map(l=>`<tr>
      ...
      <td style="white-space:nowrap;">${isEntrada(l.tipo_movimento)?'📈 Entrada':isSaida(l.tipo_movimento)?'📉 Saída':'-'}</td>
  ...
  getOpts(function(opts){
    ...
    Object.keys(porGrupo).forEach(function(gid){
      ...
      porGrupo[gid].forEach(function(r){
        const tipo=isEntrada(r.tipo_movimento)?'<span style="color:#28a745;font-size:11px;">📈 Entrada</span>':
                   isSaida(r.tipo_movimento)?'<span style="color:#dc3545;font-size:11px;">📉 Saída</span>':
